...
function showText() {
  ...
  const currentText = text[currentTextIndex.value];
  container!.innerHTML += currentText.charAt(count.value);
  count.value++;

  if (count.value == currentText.length) {
    clearInterval(timer);
...
function deleteText() {
  timer = setInterval(function () {
    ...
    if (length == 0) {
      clearInterval(timer);
      count.value = 0;
      currentTextIndex.value = Math.floor(Math.random() * text.length);
